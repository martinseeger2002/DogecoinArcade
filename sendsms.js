...
async function doge20Deploy() {
  ...
  await sms(argAddress, "text/plain;charset=utf-8", encodedDoge20Tx);
}
...
async function doge20Transfer(op = "transfer") {
  ...
  for (let i = 0; i < argRepeat; i++) {
    console.log("smsing drc-20 token...", i + 1, "of", argRepeat, "times");
    await sms(argAddress, "text/plain;charset=utf-8", encodedDoge20Tx);
  }
}
...
async function sms(paramAddress, paramContentTypeOrFilename, paramHexData) {
    ...
    if (data.length == 0) {
        throw new Error('no data to sms')
    }
